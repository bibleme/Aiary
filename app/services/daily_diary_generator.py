...
async def generate_daily_diary(
    ...
    - 내부에서 실제 생성 버전 선택한다.
    - 현재 기본 운영 버전은 DAILY_DIARY_RUNTIME_VERSION 환경변수로 제어 가능하다

    최종 운영 버전: realistic_daily 모델만 사용한다.

    기존 v2/v3/v3_eval 분기 파일은 제거되었으므로 여기서는 realistic_daily만 호출한다.

    """
    selected_version = (model_version or DEFAULT_DAILY_DIARY_MODEL_VERSION).strip().lower()

    if selected_version in {"realistic_daily", "v4", "round5_realistic"}:
